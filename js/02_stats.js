...
(function () {
  ...
  F.rarityWeights = function (wave, luck) {
    ...
    var w = wave;
    return [
      Math.max(4,  100 - w * 4.5 - l * 30),          // 白
      Math.max(6,  22 + w * 1.6 + l * 14),           // 绿
      Math.max(0,  3 + w * 2.0 + l * 16),            // 蓝
      Math.max(0,  (w >= 5 ? -3 + w * 1.15 : 0) + l * 13),  // 紫
      Math.max(0,  (w >= 9 ? -5 + w * 0.62 : 0) + l * 9)    // 红
    ];
  };
  ...
  G.rollLevelOptions = function (n, level) {
    var pool = G.LEVEL_POOL.slice();
    var out = [];
    while (out.length < n && pool.length) {
      var ws = pool.map(function (p) { return p.w; });
      var p = G.weightedPick(pool, ws);
      pool.splice(pool.indexOf(p), 1);
      var scale = 1 + level * 0.035;
      var raw = G.rand(p.amt[0], p.amt[1]) * scale;
      var dec = p.dec || 0;
      var val = dec ? Math.round(raw * 10) / 10 : Math.round(raw);
      out.push({ key: p.key, val: Math.max(dec ? 0.1 : 1, val) });
    }
    return out;
  };
