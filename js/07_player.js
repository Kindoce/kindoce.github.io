...
(function () {
  ...
  Player.prototype.updateWeapon = function (w, dt, idx) {
    ...
    w.timer = cd;
    G.Audio.sfx((def.kind === 'swing' || def.kind === 'thrust') ? 'swing' : 'fire');
  ...
  Player.prototype.draw = function (c) {
    ...
    for (var i = 0; i < this.weapons.length; i++) {
      ...
      var off = 20 + (i % 3) * 3;
      var a = w.angle;
      var pull = w.swingT > 0 ? 8 : 0;
      var wc = G.weaponIcon(w.def, w.tier, 2);
      G.PX.draw(c, wc,
        this.x + Math.cos(a) * (off + pull) + Math.cos(a + Math.PI / 2) * ((i - this.weapons.length / 2) * 5),
        this.y + Math.sin(a) * (off + pull) + Math.sin(a + Math.PI / 2) * ((i - this.weapons.length / 2) * 5) + bob,
        { rot: a + Math.PI / 4, alpha: 0.95 });
    }
  };
  ...
  function muzzle(p, a, col, n) {
    G.burst(p.x + Math.cos(a) * 18, p.y + Math.sin(a) * 18, n || 4, col, 150, { size: 2.5, lifeMul: 0.6 });
  }
