...
(function () {
  ...
  function Enemy(def, x, y, wave, bossHpMul) {
    ...
    this.kx = 0; this.ky = 0;
    this.r = def.r;
    // 精英沿用常规成长；Boss 保留固定攻防，只让生命值随波数增长。
    var hpScale = def.boss ? (bossHpMul || G.waveScale(wave).hp)
      : def.elite ? G.waveScale(wave).hp : sc.hp;
    this.maxHp = Math.max(1, Math.round(def.hp * hpScale));
    this.hp = this.maxHp;
