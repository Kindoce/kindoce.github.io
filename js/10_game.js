...
(function () {
  ...
  game.init = function () {
    ...
    window.addEventListener('keydown', function (e) {
      ...
      if (e.code === 'Escape') { e.preventDefault(); game.togglePause(); }
    });
  ...
  game.startWave = function (n) {
    ...
    this.bossHpMul = cfg.bossHpMul || 1;
    this.budget = 0;
  ...
  game.specialAlive = function () {
    for (var i = 0; i < this.enemies.length; i++) {
      var e = this.enemies[i];
      if (!e.dead && (e.def.elite || e.def.boss)) return true;
    }
    return false;
  };

  game.refreshCombatMusic = function () {
    var mode = 'normal';
    for (var i = 0; i < this.enemies.length; i++) {
      var e = this.enemies[i];
      if (e.dead) continue;
      if (e.def.boss) { mode = 'boss'; break; }
      if (e.def.elite) mode = 'elite';
    }
    if (G.Audio.getBgmState().mode !== mode) G.Audio.setCombatMode(mode);
  };

  game.updateWave = function (dt) {
    var cfg = G.WAVES[this.wave - 1];
    // 精英/Boss 战斗期间冻结波次进度，击败场上所有特殊敌人后恢复。
    var specialActive = this.specialAlive();
    if (!specialActive && this.waveTime > 0) this.waveTime -= dt;
    ...
    // 结束判定
    if (this.waveTime <= 0 && this.eliteQueue.length === 0 && !this.specialAlive()) this.endWave();
  };
  ...
  game.spawnEnemy = function (id, x, y) {
    ...
    var e = new G.Enemy(def, x, y, this.wave, this.bossHpMul);
    this.enemies.push(e);
    if (def.elite || def.boss) G.burst(x, y, 14, def.boss ? '#ff4a6b' : '#ffd24a', 200, { size: 4 });
    this.refreshCombatMusic();
    return e;
  };
  ...
  game.dropLoot = function (e) {
    var p = this.player;
    var total = Math.max(1, Math.round(e.mat * (1 + p.st.dropRate / 100)));
    var n = Math.min(10, Math.max(1, Math.ceil(total / 4)));
    var per = Math.ceil(total / n);
    for (var i = 0; i < n; i++) {
      this.pickups.push(new G.Pickup(e.x + G.rand(-8, 8), e.y + G.rand(-8, 8), 'mat', per));
    }
    // 治疗掉落
